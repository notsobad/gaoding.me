...
function markMatchingProjects(a, b) {
    var c = false,
        d = false,
        e = getChildrenOfProjectTreeObject(a),
        f;
    for (f in e) {
        var g = markMatchingProjects(e[f], b);
        c |= g.uncompletedDescendantMatches;
        d |= g.completedDescendantMatches
    }
    f = e = false;
    if (a !== null) {
        f = "nm" in a && stringMatchesSearchRegExpList(a.nm, b);
        g = "no" in a && stringMatchesSearchRegExpList(a.no, b);
        e = f || g;
        a.searchResult = {
            matches: e,
            uncompletedDescendantMatches: c,
            completedDescendantMatches: d,
            nameMatches: f,
            noteMatches: g
        };
        f = "cp" in a ? a.cp : false
    }
    return {
        uncompletedDescendantMatches: !f && e || c,
        completedDescendantMatches: f && e || d
    }
}
...
function showSiteMessageIfNeeded() {
    if (!($("body").hasClass("shared") || $("#site_message").length === 0)) if (LOCAL_STORAGE_SUPPORTED) {
        var a = $("#site_message"),
            b = a.data("messageid");
        if (localStorage.getItem("msg.closed." + b) === null) {
            ANIMATION_COUNTER.increment();
...
function closeSiteMessage() {
    var a = $("#site_message");
    if (LOCAL_STORAGE_SUPPORTED) {
        var b = a.data("messageid");
        writeLocalStorage("msg.closed." + b, "1")
    }
    ANIMATION_COUNTER.increment();
    a.slideUp(function () {
        ANIMATION_COUNTER.decrement()
