...
$(document).ready(function () {
    ...
    if (LOCAL_STORAGE_SUPPORTED) for (var a = 0; a < localStorage.length; a++) {
        var b = localStorage.key(a),
            c = localStorage.getItem(b);
        writeLocalStorageEntryToCookieIfApplicable(b, c)
    }
    createEditors();
...
function writeLocalStorageEntryToCookieIfApplicable(a, b) {
    if (a === "expanded" || a === "completedMode") $.cookie(a, b, {
        expires: 90
    })
}
...
function writeLocalStorage(a, b) {
    ...
    writeLocalStorageEntryToCookieIfApplicable(a, b)
}
...
function constructProjectTreeHtml(a, b, c) {
    ...
    return '<div class="project ' + l + '" projectid="' + d + '"><div class="dropTarget"></div><div class="highlight"></div><div class="name ' + i + '"><a class="bullet" href="' + window.location.pathname + "#/" + d + '" title="Zoom in">&bull;<div></div></a><div class="content">' + textToHtml(e) + '</div><span class="parentArrow"><span class="arrow"> > </span><span class="other"></span></span></div><div class="notes ' + j + '"><div class="content">' + textToHtml(f) + '</div></div><div class="children">' + constructChildren(a) + "</div></div>"
}
...
function markMatchingProjects(a, b) {
    var c = getChildrenOfProjectTreeObject(a),
        d = false,
        e;
    for (e in c) if (markMatchingProjects(c[e], b)) d = true;
    c = false;
    if (a != null) {
        e = "nm" in a && stringMatchesSearchRegExpList(a.nm, b);
        var f = "no" in a && stringMatchesSearchRegExpList(a.no, b);
        c = e || f;
        a.searchResult = {
            matches: c,
            descendantMatches: d,
...
function isExpandedForSearch(a) {
    return "searchResult" in a ? a.searchResult.descendantMatches : false
}
function getAdditionalClassesForSearch(a) {
    ...
    if ("searchResult" in a) {
        ...
        if (a.descendantMatches) b.project += "descendantMatches ";
...
jQuery.fn.filterVisibleProjects = function (a) {
    ...
    var b = $(this);
    a || (b = b.not(".done"));
    if (SEARCH_MODE) b = b.filter(".matches, .descendantMatches");
    return b
};
...
function getVisibleChildrenOfProjectTreeObject(a, b, c) {
    ...
    for (e in a) {
        var f = a[e],
            g = "cp" in f && f.cp;
        if ((c || !g) && (b || !SEARCH_MODE || f.searchResult.matches || f.searchResult.descendantMatches)) d.push(f)
    }
    return d
}
jQuery.fn.selectIt = function (a, b) {
    ...
    if (!c.is(".selected")) {
        ...
        if (f.length == 1) var g = f.getProject().attr("projectid"),
            k = f.find("textarea").getCaret().start;
        else g = null;
        var h = null;
        if (a == "first_child") {
        ...
        f = function () {
            ...
            if (IS_MOBILE) $(window).scrollTop(0);
                ...
                if (p != null && q.filter(p).length > 0) if (b == "animate") ANIMATION_COUNTER.cursorPos = k;
                else q.children(".name").focusContent(k);
        ...
        if (b == "animate") {
            ...
            for (o in f) {
                ...
                i = $('<div class="nameAnimated">' + m.html() + "</div>");
                $("#visible").append(i);
                i.css({
                    position: "absolute",
                    zIndex: 100,
                ...
                if (e.newOffset === null) {
                    i.offset(m.offset());
                    i.incrementAnimationCounter().animate({
                        fontSize: "0px"
                    }, ANIMATION_SPEEDS.zoom, function () {
                        ANIMATION_COUNTER.decrement()
                    })
                } else {
                    i.offset(e.newOffset);
                    j = i.position();
                    m.is(":visible") ? i.offset(m.offset()) : i.css({
                        fontSize: "0px",
                        lineHeight: "0px"
                    });
                    ...
                    e.top = j.top;
                    e.left = j.left;
                    i.incrementAnimationCounter().animate(e, ANIMATION_SPEEDS.zoom, function () {
                        ANIMATION_COUNTER.decrement()
                    })
                }
            }
...
function setTitleAndFragmentPathForProjectId(a) {
    var b = getProjectTreeObjectByProjectId(a),
        c;
    c = b === null ? SHARED_ROOT_PROJECTID !== null ? $("#workflowy").getName().children(".content").getContentText() : "整理思维." : b.nm;
...
function initializeExpandedProjects() {
    ...
    var c = null;
    if (LOCAL_STORAGE_SUPPORTED) {
        b = localStorageKey("expanded");
        c = localStorage.getItem(b)
    }
    if (!(c === null || c.length == 0)) {
        a = {};
        for (var d in PROJECTID_TO_PROJECT_MAP) {
            b = truncateProjectIdForExpandedStorage(d);
            a[b] = PROJECTID_TO_PROJECT_MAP[d]
        }
        d = c.split(",");
        for (var e in d) {
            c = d[e].split(":");
            b = truncateProjectIdForExpandedStorage(c[0]);
            c = c.length === 1 || c[1] === "e";
            if (b in a) if ("ch" in a[b]) EXPANDED_PROJECTS[b] = {
                expanded: c
            }
        }
    }
...
function saveExpanded() {
    var a = [];
    for (truncatedProjectId in EXPANDED_PROJECTS) {
        var b = EXPANDED_PROJECTS[truncatedProjectId];
        "from_server" in b && b.from_server || a.push(truncatedProjectId + ":" + (b.expanded ? "e" : "c"))
    }
    a = a.join(",");
    if (LOCAL_STORAGE_SUPPORTED) {
        b = localStorageKey("expanded");
...
function getFinishedProjectsToAnimate() {
    var a = $(".selected > .children > .project.done, .selected .project.open:visible > .children > .project.done");
...
function textToHtml(a) {
    ...
    c.sort(function (l, i) {
        if (l.spanStart == i.spanStart) return 0;
        return l.spanStart < i.spanStart ? -1 : 1
    });
    d = "";
    g = 0;
    for (var k in c) {
...
function addEvents() {
    ...
    if (!IS_MOBILE) {
        ...
        $(".editor").live("click", function () {
            var b = $(this).getContentTarget(),
                c = b.getProject();
            if (!(!c.hasClass("selected") || b.parent(".name").length != 1)) {
                if ($(this).isDoubleClick()) {
...
jQuery.fn.exportIt = function () {
    function a(k, h, n, l, i, j) {
        ...
        h = getVisibleChildrenOfProjectTreeObject(h, true, i);
        if (h.length > 0) {
            j = $("<ul />");
            n = n + 1;
            l = n > 1 ? l + c : l;
    ...
    f.append($("<div />").addClass("tagline").html('Created with <a href="http://gaoding.me/">GaoDing.me</a>'));
    b += "\n\nCreated with GaoDing.me\n";
    d = $("<pre />").text(b);
    e = $("#exportPopup");
    e.html("");
...
jQuery.fn.moveProject = function (a) {
    function b(k) {
        k = k.children(".content");
        var h = k.getEditor();
        h != false && h.is(".fixed") && k.saveContent()
    }
    var c = $(this),
        d = c.attr("projectid"),
        e = c.getParent(),
...
jQuery.fn.showSharePopup = function () {
    var a = $(this),
        ...
        d = $("#sharePopup");
    d.data("projectid", c);
    d.find("input.shareLink").val("http://gaoding.me/shared/" + c + "/");
    d.dialog("option", {
        title: "共享列表: " + htmlToText(a.getName().children(".content")).slice(0, 75) + " . . ."
    });
    d.dialog("open");
